...
void DShotR4::gpt_overflow_intr(timer_callback_args_t(*arg)) {
...
void DShotR4::gpt_gtioState_init() {
...
void DShotR4::gpt_init(void) {
  float default_duty = 0.0;
  float default_freqHz = 1.0;

  gpt_reg = (R_GPT0_Type *)((unsigned long)R_GPT0_BASE +
                            (unsigned long)(0x0100 * gpt_Channel));
  gpt_ChannelRegVal = 0x1 << gpt_Channel;
  gpt_gtioState_init();

  // reserve GPT via FspTimer class to avoid conflict with Arduino-Core
  // functions.
  fsp_timer.begin(TIMER_MODE_PWM, GPT_TIMER, gpt_Channel, default_freqHz,
                  default_duty, gpt_overflow_intr, this);
  fsp_timer.setup_overflow_irq();
  fsp_timer.set_source_start(
      GPT_SOURCE_GPT_A);  // XXX: restricted event source...
  fsp_timer.add_pwm_extended_cfg();
  ((gpt_extended_cfg_t *)fsp_timer.get_cfg()->p_extend)->gtior_setting.gtior =
      gtioStop.gtior;
  fsp_timer.open();

  // Get IRQn from current environment. need to update if fsp_timer.end() is
  // called.
...
DShotR4::DShotR4(float tr_period, float tr_t1h, float tr_t0h)
    : fsp_timer(),
      serialCore(this->fsp_timer, this->dtc_ctrl, &this->dtc_info[0],
                 this->dtc_info_len),
      blHeli(serialCore) {
...
bool DShotR4::begin(enum DShotType type, bool biDir, uint8_t channel,
                    bool useChannelA, bool useChannelB, pin_size_t pwmPinA,
                    pin_size_t pwmPinB) {
...
bool DShotR4::set_tolerance_hz(float tr_hz) {
  tolerance_hz = tr_hz;
  gpt_dshot_init();
}

bool DShotR4::set_tolerance_t1h(float tr_t1h) {
  tolerance_t1h = tr_t1h;
  gpt_dshot_init();
}

bool DShotR4::set_tolerance_t0h(float tr_t0h) {
  tolerance_t0h = tr_t0h;
  gpt_dshot_init();
}

bool DShotR4::set_rawValue(TimerPWMChannel_t channel, uint16_t value,
                           bool telemetry) {
  ...
  if ((channel == CHANNEL_A && dshotInvertA) ||
      (channel == CHANNEL_B && dshotInvertB)) {
    frame = (data << 4) | ~cksum;
  } else {
    frame = (data << 4) | cksum;
  }
...
bool DShotR4::set_throttle(TimerPWMChannel_t channel, uint16_t throttle,
                           bool telemetry) {
...
bool DShotR4::set_command(TimerPWMChannel_t channel, enum DShotCommand cmd) {
  return set_rawValue(channel, (uint16_t)cmd,
                      (cmd != DSHOT_CMD_DISARM) ? true : false);
}

bool DShotR4::set_testPattern() {
  set_rawValue(CHANNEL_A, 0x0555, true);
  set_rawValue(CHANNEL_B, 0x0AAA, false);
  return true;
}

bool DShotR4::transmit(int count) {
