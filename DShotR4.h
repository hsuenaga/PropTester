...
class DShotR4 {
 ...
 private:
  ...
  static const uint32_t pin_cfg_output_gpt =
      IOPORT_CFG_PORT_DIRECTION_OUTPUT | IOPORT_CFG_DRIVE_MID |
      IOPORT_CFG_PERIPHERAL_PIN | IOPORT_PERIPHERAL_GPT1;

  static const uint32_t pin_cfg_output_high = IOPORT_CFG_PORT_DIRECTION_OUTPUT |
                                              IOPORT_CFG_PORT_OUTPUT_HIGH |
                                              IOPORT_CFG_DRIVE_MID;

  static const uint32_t pin_cfg_output_low = IOPORT_CFG_PORT_DIRECTION_OUTPUT |
...
#error "Board is not supported."
  ...
  const static int ifgBits =
      waveformBits * 2;  // XXX: what is reasonable value???
 ...
 public:
  ...
  bool begin(enum DShotType = DSHOT150, bool biDir = false, uint8_t channel = 4,
             bool useChannelA = true, bool useChannelB = true,
             pin_size_t pwmPinA = 1, pin_size_t pwmPinB = 0);
  bool end();
  ...
  bool set_rawValue(TimerPWMChannel_t channel, uint16_t value,
                    bool telemetry = false);
  bool set_throttle(TimerPWMChannel_t channel, uint16_t throttle,
                    bool telemetry = false);
  ...
  uint32_t get_ifg_us() { return dshot_ifg_us; };

  bool get_auto_restart() { return auto_restart; };

  int get_auto_restart_count() { return auto_restart_count; };

  uint8_t get_channel() { return gpt_Channel; };

  bsp_io_port_pin_t get_bspPinA() { return bspPinA; };

  bsp_io_port_pin_t get_bspPinB() { return bspPinB; };

  pin_size_t get_pwmPinA() { return gpt_pwmPinA; };
