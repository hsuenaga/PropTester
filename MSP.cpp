...
void MSP::reply(const uint8_t *payload, uint8_t len) {
  uint8_t buf[256];
  uint8_t *p = buf;
  uint8_t checksum;
...
void MSP::proc_api_version() {
  uint8_t payload[3] = {MSP_PROTOCOL_VERSION, API_VERSION_MAJOR,
                        API_VERSION_MINOR};

  reply(payload, sizeof(payload));
}

void MSP::proc_fc_variant() {
  // fake Ardupilot, we are kind of Ardu*
  reply((uint8_t *)"ARDU", 4);
}

void MSP::proc_fc_version() {
  uint8_t payload[3] = {3, 3, 1};
  reply(payload, sizeof(payload));
}

void MSP::proc_board_info() {
  // fake ArduPilot ChibiOS
  uint8_t payload[7] = {'A', 'R', 'C', 'H', '\0', '\0', '\0'};
  reply(payload, sizeof(payload));
}

void MSP::proc_build_info() {
  // fake ArduPilot ... what is this???
  uint8_t payload[26]{0x4d, 0x61, 0x72, 0x20, 0x31, 0x36, 0x20, 0x32, 0x30,
                      0x31, 0x38, 0x30, 0x38, 0x3A, 0x34, 0x32, 0x3a, 0x32,
                      0x39, 0x62, 0x30, 0x66, 0x66, 0x39, 0x32, 0x38};
  reply(payload, sizeof(payload));
}

void MSP::proc_feature_config() {
  uint8_t payload[4] = {0, 0, 0, 0};
  reply(payload, sizeof(payload));
}

void MSP::proc_reboot() {
  // XXX: shutdown ESC serial communication here.
}

void MSP::proc_advanced_config() {
  uint8_t payload[10] = {
      1,         // gyro sync denom
      4,         // pid process denom
      0,         // use unsynced pwm,
...
void MSP::proc_uid() {
  const bsp_unique_id_t *id_ptr =
      R_BSP_UniqueIdGet();  // XXX: RA has 16 bytes identifier
  uint8_t payload[12];
  for (int i = 0; i < 12; i++) {
...
void MSP::proc_set_motor() {
  // XXX: update motor status.
  reply(NULL, 0);
}

void MSP::proc_set_passthrough() {
  uint8_t payload[1] = {
      1  // num motors
  };
...
MSP::MSP(Stream &Port, DShotR4 &DShotInstance)
    : MspPort(Port), DShot(DShotInstance) {}

MSP::~MSP() {}

bool MSP::receive(int byte) {
  switch (state) {
    case MSP_IDLE:
      if (byte == '$') {
