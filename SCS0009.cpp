...
void SCS0009::begin() { port.setTimeout(5); }

void SCS0009::end() { port.setTimeout(1000); }

uint8_t SCS0009::calcChecksum(frame_buffer_t &frame, off_t off, size_t len) {
...
uint8_t SCS0009::addChecksum(frame_buffer_t &frame, size_t len) {
...
bool SCS0009::compChecksum(frame_buffer_t &frame) {
...
bool SCS0009::readRegister(uint8_t id, address_t addr, uint8_t *buf,
                           uint8_t len) {
...
int SCS0009::read8(uint8_t id, address_t addr) {
  uint8_t byte;
...
int SCS0009::read16(uint8_t id, address_t addr) {
  uint8_t bytes[2];
...
bool SCS0009::writeRegister(uint8_t id, address_t addr, const uint8_t *buf,
                            uint8_t len) {
  frame_buffer_t frame;
  tx_header_t *thp = (tx_header_t *)&frame[0];
  uint8_t *dp = (uint8_t *)(thp + 1);
...
ssize_t SCS0009::write8(uint8_t id, address_t addr, uint8_t val) {
...
ssize_t SCS0009::write16(uint8_t id, address_t addr, uint16_t val) {
  uint8_t buffer[2];
...
int SCS0009::readVersion(uint8_t id) { return read16(id, MAJOR_VERSION); }

int SCS0009::readServoVersion(uint8_t id) { return read16(id, MASTER_VERSION); }

int SCS0009::readBaud(uint8_t id) { return read8(id, BAUD); }

int SCS0009::writeBaud(uint8_t id, baud_t baud) {
...
int SCS0009::getPosition(uint8_t id) { return read16(id, CURRENT_POSITION); }

int SCS0009::setPosition(uint8_t id, uint16_t pos) {
...
int SCS0009::getDegree(uint8_t id) { return posToDeg(getPosition(id)); }

int SCS0009::setDegree(uint8_t id, uint16_t deg) {
  return setPosition(id, degToPos(deg));
}

SCS0009::config_t SCS0009::getConfig(uint8_t id) {
...
SCS0009::status_t SCS0009::getStatus(uint8_t id) {
