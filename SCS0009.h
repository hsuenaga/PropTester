...
class SCS0009 {
 ...
 private:
  ...
  bool writeRegister(uint8_t id, address_t addr, const uint8_t *buf,
                     uint8_t len);
 ...
 public:
  ...
  static inline int FutabaToPos(int us, bool fullrange = false,
                                bool offJR = false) {
    return degToPos(FutabaToDeg(us, offJR) * (fullrange ? 5 : 1));
  };

  static inline int JRToDeg(int us) { return pwmToDeg(us, 1500, 400); };

  static inline float JRToDegF(int us) { return pwmToDegF(us, 1500, 400); };
